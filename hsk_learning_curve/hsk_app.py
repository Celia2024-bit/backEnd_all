...
def get_user_progress():
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            if level:
                cur.execute(
                    "SELECT level, record FROM user_progress WHERE username = %s AND level = %s AND record IS NOT NULL",
                    (username, level)
                )
            else:
                cur.execute(
                    "SELECT level, record FROM user_progress WHERE username = %s AND record IS NOT NULL",
                    (username,)
                )
            rows = cur.fetchall()

    progress_map = {str(r['level']): json.loads(r['record']) if isinstance(r['record'], str) else r['record'] for r in rows}
...
def get_user_mastery():
    ...
    for r in rows:
        ...
        mastery[key] = json.loads(r['record']) if isinstance(r['record'], str) else r['record']
...
def save_progress():
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            """, (username, level, json.dumps(record)))
        conn.commit()
...
def save_mastery():
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            """, (username, char, level, json.dumps(record)))
        conn.commit()
